<div>   
      ...
      <form wire:submit.prevent="onCsvToJson">
        ...
        <div class="form-group position-relative mb-3">
            <textarea id="input" class="form-control" wire:model.defer="csv" rows="10" placeholder="{{ __('Paste your content here...') }}" required></textarea>
    ...
    document.addEventListener('livewire:load', function () {
      ...
      el.addEventListener('click', function() {
        if (el.innerHTML === clearIcon) {
          // Clear action
          @this.set('csv', ''); // Update Livewire state
          setPasteIcon();
        } else {
          // Paste action
          navigator.clipboard.readText().then(function(clipText) {
            @this.set('csv', clipText);
            setClearIcon();
          }).catch(function() {
