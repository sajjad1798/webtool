      ...
      <form wire:submit.prevent="onPaceConverter">
        ...
        <div class="form-group mb-3">
            ...
            <select wire:model.defer="convert_from" class="form-control form-select">
                <option value="minute_per_kilometer">{{ __('Minute per Kilometer (min/km)') }}</option>
                <option value="second_per_minute">{{ __('Second per Minute (s/m)') }}</option>
                <option value="minute_per_mile">{{ __('Minute per Mile (min/mi)') }}</option>
            </select>
        </div>
