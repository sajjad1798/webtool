      ...
      <form wire:submit.prevent="onReactivePowerConverter">
        ...
        <div class="form-group mb-3">
            ...
            <select wire:model.defer="convert_from" class="form-control form-select">
                <option value="volt">{{ __('Volt-Ampere Reactive (VAR)') }}</option>
                <option value="millivolt">{{ __('Millivolt-Ampere Reactive (mVAR)') }}</option>
                <option value="kilovolt">{{ __('Kilovolt-Ampere Reactive (kVAR)') }}</option>
                <option value="megavolt">{{ __('Megavolt-Ampere Reactive (MVAR)') }}</option>
                <option value="gigavolt">{{ __('Gigavolt-Ampere Reactive (GVAR)') }}</option>
            </select>
        </div>
