<div>
      ...
      <form wire:submit.prevent="onBinaryToDecimal">
        ...
        <div class="form-group position-relative mb-3">
            <textarea id="input" class="form-control" wire:model.defer="binary" placeholder="{{ __('Paste your content here...') }}" rows="10" required></textarea>
        ...
        <div class="form-group text-center">
            <button class="btn btn-info w-md-100 mb-1 mb-md-0" wire:loading.attr="disabled">
              <span>
                <div wire:loading.inline wire:target="onBinaryToDecimal">
                  <x-loading />
                </div>
      ...
      document.addEventListener('livewire:load', function () {
        ...
        el.addEventListener('click', function() {
          if (el.innerHTML === clearIcon) {
            // Clear action
            @this.set('binary', ''); // Update Livewire state
            setPasteIcon();
          } else {
            // Paste action
            navigator.clipboard.readText().then(function(clipText) {
              @this.set('binary', clipText);
              setClearIcon();
            }).catch(function() {
